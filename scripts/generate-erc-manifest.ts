...
const configMap: any = {
  ...
  packageLogsLevel: `ENV.${buildEnvVar('LOGS_LEVEL')}||warn`,
...
const result = initConfig(configMap, {
  ...
  descriptions: {
    ...
    [`${buildEnvVar('LOGS_LEVEL')}`]: 'Package log threshold (canonical): off|none|silent|error|warn|info|debug|verbose — omit both LOGS_LEVEL and LOG_LEVEL keys = warn; see docs/package-usage.md',
    [`${buildEnvVar('LOG_LEVEL')}`]: 'Legacy minimum log level — used only when LOGS_LEVEL is not set in the environment',
